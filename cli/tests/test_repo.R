...
test_that_managed("Test repo init", {
  repo_path <- create_test_repo("repo")
  retcode <- rsuite_run(args = c("repo", "init", "-v", "-d", repo_path, "-b", "TRUE"),
                        wd = repo_path)
  expect(retcode == 0, sprintf("Failed to initialize repository: %s", repo_path))
})
