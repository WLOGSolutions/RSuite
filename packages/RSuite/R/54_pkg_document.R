...
pkg_build_docs <- function(pkg_name, pkg_path, rver, libpath) {
  ...
  if (file.exists(ns_path)) {
    ns_lines <- trimws(readLines(ns_path))
    ns_lines <- ns_lines[ns_lines != ""]
    if (!any(grepl("^# Generated by roxygen", ns_lines[1]))) {
      build_ns <- FALSE # not generated with roxygen
    } else {
      build_ns <- length(ns_lines[-1]) == 0 # do not rebuild if not empty: roxygen will do it
    }
  } else {
    build_ns <- TRUE
  }
...
validate_package_imports <- function(pkg_name, pkg_path) {
  ...
  desc_not_ns <- setdiff(desc_not_ns, base_pkgs)
