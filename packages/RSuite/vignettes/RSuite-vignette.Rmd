...
The lock file is in the <my_project>/deployment directory under the 'env.lock' name. It is a dcf file that stores information about packages in the local environment together with their versions. A sample record from the 'env.lock' file is presented below:
...
When dependencies are being installed using ```RSuite::prj_install_deps()``` the 'env.lock' file will be used to detect whether any package will change versions. If that's the case a warning message will be displayed like this:

```
rsuite: The following packages will be updated from last lock: colorspace
```

The feature allows preventing errors cause by newer versions of packages which might work differently than previous versions used in the project.
