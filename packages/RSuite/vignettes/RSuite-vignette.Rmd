...
```{r}
prj <- RSuite::prj_start(name = 'my_project', path = tempdir())
```

The RSuite project is beeing created in tempdir which is not under git/svn control. This is the cause of warning you can see
above.
...
```
setwd(file.path(tempdir(), "my_project"))
```

Lets see the contents of the project folder created:

```{r}
cat(list.files(prj$path, all.files = TRUE), sep = "\n")
```
...
master_file <- file.path(prj$path, "R", "master.R")
cat(readLines(con = master_file), sep = "\n")
```

To check if everything is working properly run the ```R/master.R``` script:

```{r}
setwd(file.path(prj$path))
source("R/master.R")
...
```{r}
RSuite::prj_start_package(name = "mypackage", prj = prj, skip_rc = TRUE)
```
...
```{r comment = '', echo = FALSE}
src_file <- file.path(prj$path, "packages", "mypackage", "R", "hello_world.R")
writeLines(c("#' @export",
...
cat(readLines(con = src_file), sep = "\n")
```

Please remember to add ```#' @export```. It is requered to expose the function into global namespace.
...
```{r comment='', echo = FALSE}
desc_file <- file.path(prj$path, "packages", "mypackage", "DESCRIPTION")
dcf <- read.dcf(file = desc_file)
dcf <- cbind(dcf, Depends = "data.table (>= 1.10.1)")
write.dcf(dcf, file = desc_file)

cat(readLines(con = desc_file), sep = "\n")
```
...
```{r}
devtools::load_all(file.path(prj$path, "packages", "mypackage"))
```

Lets see how ```hello_world``` function behaves now:
```{r}
mypackage::hello_world("John")
```
...
lines <- c(lines[c(1:15)],
           ...
           "")
writeLines(lines, con = master_file)

cat(readLines(master_file), sep = "\n")
```
...
```{r comment = "", echo = FALSE}
config_file <- file.path(prj$path, "config.txt")
writeLines("LogLevel: DEBUG", con = config_file)
...
```{r}
list.files(path = file.path(prj$path, "logs"))
```

When you open this log in an editor you should see content similar to this

```{r echo = FALSE}
log_file <- list.files(path = file.path(prj$path, "logs"), pattern = "*.log", full.names = TRUE)[1]
...
```{r comment = "", echo = FALSE}
hello_file <- file.path(prj$path, "packages", "mypackage", "R", "hello_world.R")
writeLines(c("#' @export",
             "hello_world <- function(name) {",
...
```{r}
RSuite::prj_zip(prj, path = tempdir())
```
...
```{r}
RSuite::prj_zip(prj, path = tempdir(), zip_ver = '1.0')
```
